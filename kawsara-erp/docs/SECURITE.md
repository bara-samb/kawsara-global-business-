...
- Verrouillage de compte apres 5 echecs de connexion consecutifs (15 minutes), avec notification
  aux administrateurs (`src/lib/auth.ts`, `src/lib/notify.ts`).
...
**Limite connue** : les sessions sont des JWT stateless. Il n'existe pas de liste de sessions
actives revocables individuellement cote serveur (necessiterait de passer a des sessions en base
de donnees). La revocation actuelle se fait en desactivant le compte (`user.active = false`), ce
qui invalide l'acces a la prochaine verification serveur.
...
- CAPTCHA maison (defi mathematique signe par HMAC-SHA256, sans etat serveur ni service tiers)
  sur les formulaires de connexion et d'inscription (`src/lib/captcha.ts`).
- Limitation de debit (rate limiting) en memoire sur `/api/auth/callback/credentials` et
  `/inscription` : 15 tentatives par minute et par IP (`src/proxy.ts`). Adaptee a une instance
...
- Consultable par les roles autorises sur `/erp/securite`.
